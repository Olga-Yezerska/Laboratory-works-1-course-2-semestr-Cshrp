...
public abstract class OrganizationalStructure
{
    ...
    {
        string partnershipActPath = @"D:\Новая папка\C#\Lab5Ver2\partnership_act.txt"; //шлях до файлу
    ...
    {
        string filePath = $@"D:\Новая папка\C#\Lab5Ver2\update_{structureName}.txt"; //шлях до файлу
    ...
    /// <summary>
    /// Метод запису результат поліморфізму
    /// </summary>
    /// <param name="service"> сервіс </param>
    /// <param name="operationType"> тип операції </param>
    /// <param name="details"> деталі та результат </param>
    /// <param name="name"> назва структури </param>
    public void LogOperation(Service service, string operationType, string details, string name)
    {
        string filePath = $@"D:\Новая папка\C#\Lab5Ver2\operations_{name}.txt"; //шлях до файлу
        string logEntry = $"\n=== Operation Log ===\n";
        ...
        string updatedData = existingData + logEntry; //оновлення даних 
        service.Data = updatedData;
        service.WriteData(); //запис даних у файл
    }
}