...
public partial class Department
{
    ...
    /// <summary>
    /// Метод визначення випадкової кількості годин для дисциплін із заданою загальною сумою (кратне 30).
    /// </summary>
    public void SetDisciplinesHours()
    {
        int targetTotalHours = 7200; //загальна кількість годин
        int minHours = 60; //мінімальна кількість годин
        int remainingHours = targetTotalHours - (40 * minHours); //залишок годин
        disciplineHours = new int[40]; //масив годин дисциплін
        //призначення мінімальної кількісті годин
        for (int i = 0; i < 40; i++)
        {
            disciplineHours[i] = minHours;
        }
        //розподіл залишку годин випадково
        while (remainingHours > 0)
        {
            int index = random.Next(40); //випадковий індекс
            int hoursToAdd = Math.Min(random.Next(1, 6) * 30, remainingHours); 
            disciplineHours[index] += hoursToAdd;
            remainingHours -= hoursToAdd;
        }
        //перевірка на кратність 30
        if (remainingHours != 0)
        {
            int index = random.Next(40);
            disciplineHours[index] += remainingHours;
        }
    }
    /// <summary>
    ...
    /// <summary>
    /// Метод формування розподілу дисциплін по семестрах з рівномірним розподілом годин
    /// </summary>
    /// <returns>Розподіл дисциплін у вигляді рядка</returns>
    public string DisciplineDistribution()
    {
        string output = "=== Discipline Distribution ===\n";
        int totalSemesters = 8; //кількість семестрів
        int targetHoursPerSemester = 900; //годин на семестр
        int disciplineIndex = 0; //індекс поточної дисципліни
        for (int year = 1; year <= 4; year++)
        {
            ...
            for (int semester = (year - 1) * 2 + 1; semester <= year * 2; semester++)
            {
                ...
                int semesterHours = 0; //загальна кількість годин у семестрі
                //додавання дисципліни до поточного семестру, поки не досягнемо 900 годин
                while (disciplineIndex < disciplines.Length && semesterHours + disciplineHours[disciplineIndex] <= targetHoursPerSemester)
                {
                ...
                //якщо залишилися години, додаємо частину наступної дисципліни
                if (disciplineIndex < disciplines.Length && semesterHours < targetHoursPerSemester)
                {
                    int remainingHours = targetHoursPerSemester - semesterHours;
                    output += $"    - {disciplines[disciplineIndex]}: {remainingHours} hours (partial)";
                    output += $", Lecturer: {disciplineLecturers[disciplineIndex]}\n";
                    disciplineHours[disciplineIndex] -= remainingHours;
                    semesterHours += remainingHours;
                }
