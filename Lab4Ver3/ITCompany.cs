...
public class ITCompany : OrganizationalStructure
{
    ...
    {
        service.PathToFile = @"D:\Новая папка\C#\Lab4Ver3\setup_data.txt"; //шлях до файлу
    ...
    public void CreateRequest(string interactionFormat, string[] directions, string benefits)
    {
        ...
        service.PathToFile = @"D:\Новая папка\C#\Lab4Ver3\partnership_request.txt";
        service.WriteData();
    }
    /// <summary>
    ...
    public void ProvideRecommendations(double fact, double minimum, double norm)
    {
        ...
        service.PathToFile = @"D:\Новая папка\C#\Lab4Ver3\recommendations.txt";
        service.WriteData();
    }
    /// <summary>
    /// Метод зміни назви компанії
    /// </summary>
    /// <param name="newName"> нова назва </param>
    public override void ChangeName(string newName)
    {
        base.ChangeName(newName); //виклик методу базового класу
        service.PrintData($"ITCompany name updated to: {newName}\n");
    }
}